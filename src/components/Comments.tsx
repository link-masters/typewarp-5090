...
function CommentCard({
  ...
  return (
    <div
      ...
      } p-6 bg-bg-card border border-neutral-200 dark:border-white/5 relative group rounded-xl`}
    >
      <div className="flex items-start gap-4">
        {/* Avatar */}
        <div
          className={`w-10 h-10 border flex items-center justify-center font-mono font-black shrink-0 rounded-lg ${
            comment.author.includes("TypeWarp")
              ? "border-accent-glitch bg-accent-glitch/10 text-accent-glitch"
              : "border-neutral-200 dark:border-white/10 bg-neutral-100 dark:bg-white/5 text-neutral-400 dark:text-white/40"
          }`}
        >
          {comment.avatar}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 mb-3 font-mono">
            ...
            {comment.author.includes("TypeWarp") && (
              <span className="px-2 py-0.5 bg-accent-glitch text-black text-[8px] font-black uppercase tracking-tighter rounded">
                CORE_ADMIN
              </span>
            )}
          ...
          <div className="flex items-center gap-6 font-mono">
            ...
            {!isReply && (
              <button
                ...
                className="flex items-center gap-2 text-[10px] uppercase tracking-widest text-text-muted hover:text-text-primary transition-colors"
              >
          ...
          {showReplyInput && (
            <div className="mt-6 flex gap-3">
              <input
                ...
                className="flex-1 px-4 py-3 bg-bg-void border border-neutral-200 dark:border-white/10 text-text-primary font-mono text-xs outline-none focus:border-accent-glitch/50 rounded-lg"
              />
              <button className="px-4 py-3 bg-accent-glitch text-black font-black text-[10px] uppercase tracking-widest active:scale-95 transition-all rounded-lg">
                Send
              </button>
            </div>
          )}
    ...
    <section className="mt-20 pt-16 border-t border-neutral-200 dark:border-white/5">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12 font-mono">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 border border-accent-glitch/20 bg-accent-glitch/5 flex items-center justify-center rounded-xl">
        ...
        <div className="flex items-center gap-1 p-1 bg-neutral-100 dark:bg-white/5 border border-neutral-200 dark:border-white/5 rounded-lg">
          <button
            onClick={() => setSortBy("popular")}
            className={`px-4 py-2 text-[9px] font-black uppercase tracking-widest transition-all rounded-md ${
              sortBy === "popular"
      ...
      <form onSubmit={handleSubmit} className="mb-12">
        <div className="p-8 bg-bg-card border border-neutral-200 dark:border-white/5 group relative rounded-xl">
          ...
          <textarea
            ...
            className="w-full p-4 bg-bg-void border border-neutral-200 dark:border-white/10 text-text-primary font-mono text-sm outline-none focus:border-accent-glitch/50 resize-none min-h-[140px] rounded-lg"
          />
          <div className="flex flex-col sm:flex-row items-center justify-between mt-6 gap-4">
            ...
            <button
              ...
              className="w-full sm:w-auto px-8 py-4 bg-accent-glitch text-black font-black text-[10px] uppercase tracking-[0.4em] transition-all disabled:opacity-30 flex items-center justify-center gap-3 active:scale-95 rounded-lg"
            >
      ...
      {/* Load More */}
      <div className="mt-12">
        <button className="w-full py-4 border border-neutral-200 dark:border-white/10 font-mono text-[9px] text-text-muted uppercase tracking-[0.5em] hover:border-accent-glitch/30 hover:text-text-primary transition-all rounded-xl">
